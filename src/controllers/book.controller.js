...
class BookController extends BaseController {
  ...
  async createBook(req, res) {
    try {
      ...
      logger.error('Error creating book', { error: error.message, stack: error.stack });
  ...
  async listBooks(req, res) {
    try {
      ...
      const result = await this.listBooksUseCase.execute(filters, pagination, user);
      ...
      return this.handleSuccess(res, formattedResponse, 'Books retrieved successfully');
    } catch (error) {
  ...
  async updateBookContent(req, res) {
    try {
      ...
      logger.error('Error updating book content', { error: error.message, bookId: req.params.id });
      return this.handleError(res, error);
    }
  }
  ...
  async getBookContent(req, res) {
    try {
      ...
      return this.handleSuccess(res, { content }, 'Book content retrieved successfully');
    } catch (error) {
      logger.error('Error fetching book content', { error: error.message, bookId: req.params.id });
      return this.handleError(res, error);
    }
  }
  ...
  async publishBook(req, res) {
    try {
      ...
      logger.info('Publishing book', { bookId: id, userId: user.id, publishingMethod });

      const book = await this.publishBookUseCase.execute(id, { publishingMethod, releaseDate }, user);

      return this.handleSuccess(res, book, 'Book published successfully');
    } catch (error) {
      logger.error('Error publishing book', { error: error.message, bookId: req.params.id });
      return this.handleError(res, error);
    }
  }
  ...
  async getBookStatus(req, res) {
    try {
      ...
      return this.handleSuccess(res, status, 'Book status retrieved successfully');
    } catch (error) {
      logger.error('Error fetching book status', { error: error.message, bookId: req.params.id });
      return this.handleError(res, error);
    }
  }
}
